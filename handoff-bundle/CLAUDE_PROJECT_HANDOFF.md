...
- `NOTE_EMAIL` / `NOTE_PASSWORD`（B案 note自動投稿用）
...
- [ ] `compile_video.mjs` 本実装（ffmpeg連携）
- [ ] `upload_youtube.mjs` 本実装（OAuth refresh tokenフロー）
- [ ] GitHub Issues 自動起票（gh CLI ベース）
...
- **GitHub Issues**: 自動起票（要本実装、workflow末尾に追加予定）
