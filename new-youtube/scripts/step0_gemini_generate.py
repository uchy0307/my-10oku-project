...
# 2026-05-20 ROOT FIX:
#  - deprecated モデル削除: gemini-1.5-flash / gemini-1.5-flash-8b (404/サポート終了)
#  - Free Tier に残る安定モデル + 2.5-pro を末尾に追加（pro は無料 quota あり最後の砦）
...
【尺の必須条件 — 最優先】
- TTS 換算で 33 分以上の本編にすること
- 全 8 章の narration を合算した本文は、句読点・改行・空白を除いた可視文字で
  **3,630 字以上** を必須とする (理想は 9,600〜12,000 字)
- 章あたりの目安は narration 1,200〜1,500 字。1 章でも 800 字未満は NG
- この尺条件を満たさない出力は破棄・再生成対象になる

【ルール】
- 対象は大人の対人心理学・コミュニケーション。性的・未成年関連の話題は含めない。
- テーマ範囲: 恋愛心理 / 職場の人間関係 / 女性のコミュニケーション術 / 認知心理学的な学習 /
  仕事と恋愛の両立 / 大人の自己成長戦略 / 人間関係改善法 / 大人の品格・所作・ライフスタイル
...
【最終再確認 — 出力前に必ずセルフチェック】
- 章数: ちょうど 8 章 (id=1..8)
- 各章 narration: 句読点除き 460 字以上 (理想 1,200〜1,500 字)、800 字未満は破棄対象
- 合計 narration: 句読点除き 3,630 字以上 (理想 9,600〜12,000 字、TTS 33 分以上)
- 性的描写・未成年関連・特定個人への中傷は一切含めない
- JSON はコードフェンス無しの素の JSON、最後の閉じカッコ } まで必ず出力する
- 上記いずれかを欠く出力は post-validator が ValueError を投げて自動破棄→再生成される
"""
    ...
    for model_idx, model_name in enumerate(models_to_try):
        ...
        for attempt in range(3):
            try:
                ...
                r.raise_for_status()
                data = r.json()
                # 2026-05-20 ROOT FIX: safety filter 発火時 candidates が空 or finishReason=SAFETY/BLOCKED.
                # KeyError で死ぬのではなく原因を log → 次モデルに fallback。
                candidates = data.get("candidates")
                if not candidates:
                    pf = data.get("promptFeedback") or {}
                    print(
                        f"[WARN] gemini model={model_name} returned no candidates. "
                        f"promptFeedback={pf}"
                    )
                    last_err = Exception(f"no candidates (safety filter?) on {model_name}")
                    break  # 同 model でリトライしても無駄、次モデルへ
                ...
                if not parts:
                    print(
                        f"[WARN] gemini model={model_name} empty parts. "
                        f"finishReason={finish_reason} safetyRatings={cand0.get('safetyRatings')}"
                    )
                    last_err = Exception(
                        f"empty parts (finishReason={finish_reason}) on {model_name}"
                    )
                    break
                if model_idx > 0:
...
# 2026-05-20: prompt 字数条件 (>= 3,630字) に teeth を持たせるための post-validation.
# 句読点・記号・空白を除いた可視文字で計数する.
MIN_NARRATION_CHARS = 3630
_PUNCT_EXCLUDE_PATTERN = re.compile(
    r"[\s　、。．，！？!?「」『』（）()【】［］\[\]・…―ー〜~“”\"'：；:;／/＼\\]"
)


def _count_narration_chars(obj: dict) -> int:
    chapters = obj.get("chapters", []) or []
    total = 0
    for ch in chapters:
        nar = (ch.get("narration") or "") if isinstance(ch, dict) else ""
        total += len(_PUNCT_EXCLUDE_PATTERN.sub("", nar))
    return total


def _assert_min_narration(obj: dict) -> None:
    n = _count_narration_chars(obj)
    if n < MIN_NARRATION_CHARS:
        raise ValueError(
            f"narration too short: {n} chars < {MIN_NARRATION_CHARS} (TTS 33min equivalent). "
            "Regenerate with longer chapters."
        )
    print(f"[step0][len] OK narration chars={n} (>= {MIN_NARRATION_CHARS})")
...
def _generate_once(topic, out_dir, attempt):
    ...
    user = (
        f"今回のテーマ: 「{topic}」\n"
        f"上記スキーマに従う JSON のみ出力してください。\n"
        f"※ 8 章の narration 合算は句読点除き {MIN_NARRATION_CHARS} 字以上 (TTS 33分以上) を必須とします。\n"
        f"[試行{attempt}]"
    )
    raw = call_gemini(user)
    raw = _strip_codefence(raw)
    obj = json.loads(raw)
    # 字数 gate: 未達なら ValueError → 上位 generate_script が再試行
    _assert_min_narration(obj)
