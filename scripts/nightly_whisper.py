...
def main():
    ...
    for kind, audio_dir in targets:
        ...
        # 1KB 未満の SRT は未完成扱い (whisper 失敗で空生成 or refine 前)
        srts_ok = [p for p in audio_dir.glob("*.srt") if p.stat().st_size >= 1000]
        missing = len(mp3s) - len(srts_ok)
        log(f"  {kind}: {missing} files need whisper ({len(srts_ok)}/{len(mp3s)} done)")
        if missing == 0:
            continue
